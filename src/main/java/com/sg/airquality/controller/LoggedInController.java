...
import static com.sg.airquality.controller.BaseController.dataLoadError;
import static com.sg.airquality.controller.BaseController.searchResultsNames;
import com.sg.airquality.entity.Account;
import com.sg.airquality.entity.Datum;
import com.sg.airquality.entity.Report;
import com.sg.airquality.entity.SearchResult;
...
public class LoggedInController {
    ...
    static String defaultCity = "";          // Load from from baseController if applicable

    static String username = "";           // Filled in by logging in (or creating account)
    
    // Default page. Load in any necessary information 
    @GetMapping("loggedInDefault")
    public String loggedInDefault(HttpServletRequest request, Model model){
        ...
        // Load info from the report
        model.addAttribute("dataLoadError", dataLoadError);  // In case no data could be pulled up
        ...
        return "loggedIn";              // Refresh the page
    }
    ...
    // Load the report from the default station (if there is oen)
    @GetMapping("loadDefaultStationReport")
    public String loadDefaultStationReport(HttpServletRequest request, Model model) {

        // If defaultCity is empty, then just go to the base page. 
        if( defaultCity.equals("") ){
            
            // Clear out serach results, report
            searchResultsNames = new ArrayList<String>();
            co = "";
            h = "";
            no2 = "";
        ...
        // Otherwise, load info into the searchResultsNames and the report
        // Get the results of the search
        searchResults = getStations(defaultCity);
        ...
        searchResultsNames.remove(defaultCity);
        searchResultsNames.add(0, defaultCity);
        
        
        // Get the url for that station name. Construct the http request
        String stationUrl = searchResults.get(defaultCity);
    ...
    @GetMapping("loadStationReport2")
    public String loadStationReport2( HttpServletRequest request ){
        
        // Get the user's station selection from the dropdown menu
        String stationName = request.getParameter("stationName");
         
        // If checkmark is checked, save this to defaultCity in the database 
        String defaultCityOn = request.getParameter("defaultCityOn"); 
        if( defaultCityOn != null && defaultCityOn.equals("defaultCityOn") ){           // null means not checked
            s.setDefaultCity( username, stationName);
        }
